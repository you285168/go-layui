...
func newMergely(e *ElemBase, lf, rf string, onf OnGetFile) *UIMergely {
	m := &UIMergely{e, lf, rf, onf}
	m.self = m
	return m
}

func NewMergely(lf, rf string, onf OnGetFile) *UIMergely {
	return newMergely(newElem("mergely", HtmlMergely), lf, rf, onf)
}

func (m *UIMergely) Clone() HtmlElem {
	nm := newMergely(cloneElem(m.Id, "mergely", HtmlMergely), m.LeftFile, m.RightFile, m.F)
