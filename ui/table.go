...
type OnTableUsrEvent func(user string, event int, cols []string) *Response
...
func newTable(e *ElemBase, header []string) *UITable {
	h := make([]string, len(header))
	copy(h, header)
	t := &UITable{e, h, make([]int, len(header)), false, false, true, nil, nil, nil, nil, []int{0}, false, nil, nil, make([]int, len(header)), make([]string, len(header))}
	t.self = t
	return t
}

func NewStaticTable(header []string, data [][]string) *UITable {
	t := newTable(newElem("table", HtmlTable), header)
	t.data = data
	return t
}

func NewTable(header []string, gd OnTableGetData) *UITable {
	t := newTable(newElem("table", HtmlTable), header)
	t.FuncData = gd
	return t
}

func NewToolTable(search bool, header []string, gd OnTableGetData, event OnTableEvent) *UITable {
...
func NewUserDefineTable(search bool, header, usrBtName []string, gd OnTableGetData, event OnTableUsrEvent) *UITable {
	t := NewTable(header, gd)
	t.Tool = true
	t.Search = search
...
func (table *UITable) Render() string {
	return strings.Replace(TempTable, "__TABLEID__", table.ID(), -1) + table.ElemBase.Render()
}

func (table *UITable) SetData(data [][]string) {
	table.data = data
}
func (table *UITable) SetColumnType(index int, c TableColumnType) {
	table.ColType[index] = int(c)
}
func (table *UITable) SetUrlHandler(FuncUrl OnTableUrl) {
	table.FuncUrl = FuncUrl
}
func (table *UITable) SetPage(b bool) {
	table.Page = b
}
func (table *UITable) SetTool(b bool) {
	table.Tool = b
}
func (table *UITable) SetKey(key []int) {
	table.key = key
}
func (table *UITable) SetColumnSort(index int, s bool) {
	i := 0
	if s {
		i = 1
	}
...
// 第二列如果值大于0则设置颜色
func (table *UITable) SetColumnTemplate(index int, function string) {
	table.ColTemplate[index] = function
}

func (table *UITable) TableGetData(user string, page, limit int, searchtxt string) (total int, data [][]string) {
	all := table.data
	if searchtxt != "" {
		all = make([][]string, 0)
...
func (table *UITable) TableEvent(user string, t TableOperType, cols []string) ApiRsp {
