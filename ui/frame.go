...
func NewFrame(router, title, icon string, onload OnFrameLoad) *Frame {
	f := &Frame{router, title, icon, make([]HtmlElem, 0), make(map[string]HtmlElem, 0), onload, nil}
	f.uiFactory = NewFactory(f)
	return f
}
func (f *Frame) AddElem(e HtmlElem) UIFactory {
